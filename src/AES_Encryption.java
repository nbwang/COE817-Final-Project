...
public class AES_Encryption {
	...
	public static void main(String[] args) throws UnsupportedEncodingException {
		...
		if(plainText.length() == 16){
			...
			// Run AES 10 rounds
			state = add_roundKey(state, 0);
			
			for(int round = 1; round < 10; round++){
				state = sub_bytes(state);
				state = shift_rows(state);
				state = mix_columns(state);
				state = add_roundKey(state, round);
			}
			
			state = sub_bytes(state);
			state = shift_rows(state);
			state = add_roundKey(state, 10);
	...
	// AES Sub Bytes method
	public static byte[][] sub_bytes(byte[][] state_in){
        ...
        for (column = 0; column < 4; column++){
            state[0][column] = (byte) (S_box[Integer.parseInt(Character.toString(state_in_string[0][column].charAt(0)),16)][Integer.parseInt(Character.toString(state_in_string[0][column].charAt(1)),16)]);
            state[1][column] = (byte) (S_box[Integer.parseInt(Character.toString(state_in_string[1][column].charAt(0)),16)][Integer.parseInt(Character.toString(state_in_string[1][column].charAt(1)),16)]);
            state[2][column] = (byte) (S_box[Integer.parseInt(Character.toString(state_in_string[2][column].charAt(0)),16)][Integer.parseInt(Character.toString(state_in_string[2][column].charAt(1)),16)]);
            state[3][column] = (byte) (S_box[Integer.parseInt(Character.toString(state_in_string[3][column].charAt(0)),16)][Integer.parseInt(Character.toString(state_in_string[3][column].charAt(1)),16)]);
        }
        return state;
    }
